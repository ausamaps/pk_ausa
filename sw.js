...
// Fetch: estrategia network-first para datos dinámicos, cache-first para assets
self.addEventListener('fetch', event => {
  const url = new URL(event.request.url);

  // Para Leaflet tiles, CDNs y recursos: cache-first
    ...
    event.respondWith(
      caches.match(event.request).then(response => {
        return response || fetch(event.request).then(response => {
          ...
          // Si falla y no hay cache, devolver error offline genérico
